...
import { escape } from 'neo-blessed';
  ...
  while (match = text.match(Markup.TAG_RE)) {
    ...
    if (!match[1]) { // open tag
      ...
      if (replace) {
        ...
        var lastChild = last(parent.contents);
        parent.push(child);
        stack.push(lastChild === last(parent.contents) ? lastChild : child);
      }
    } else { // close tag
      var closed;
...
Markup.prototype.toString = function () {
  return this.style + this.contents.map(function (item) {
    return typeof item === 'string' ? escape(item) : item;
